...
public class ScopeHandle {
  ...
  convenience init(parentIdentifier: ScopeIdentifier, state: Any?, stateUpdater: @escaping (ScopeIdentifier, Any?) -> ()) {
    self.init(identifier: parentIdentifier.append(__currentIdentifier.update({ (val: Int32) -> Int32 in
      return val + 1
    })), state: state, stateUpdater: stateUpdater)
  }
  
  init(identifier: ScopeIdentifier, state: Any?, stateUpdater: @escaping (ScopeIdentifier, Any?) -> ()) {
