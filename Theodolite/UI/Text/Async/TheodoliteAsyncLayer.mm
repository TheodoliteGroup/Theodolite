...
@implementation TheodoliteAsyncLayer
{
...
+ (dispatch_queue_t)displayQueue
{
  ...
  dispatch_once(&onceToken, ^{
    displayQueue = dispatch_queue_create("com.facebook.TheodoliteAsyncLayer.display", DISPATCH_QUEUE_CONCURRENT);
...
+ (id)defaultValueForKey:(NSString *)key
{
  if ([key isEqualToString:@"displayMode"]) {
    return @(TheodoliteAsyncLayerDisplayModeDefault);
  } else {
...
- (void)display
{
  ...
  if (!_needsAsyncDisplayOnly) {
    switch (self.displayMode) {
      case TheodoliteAsyncLayerDisplayModeDefault:
        parentTransactionContainer = self.ck_parentTransactionContainer;
        renderSynchronously = (parentTransactionContainer == nil);
        break;
      case TheodoliteAsyncLayerDisplayModeAlwaysAsync:
        parentTransactionContainer = self.ck_parentTransactionContainer;
        break;
      case TheodoliteAsyncLayerDisplayModeAlwaysSync:
...
#pragma clang diagnostic pop
  ...
  ck_async_transaction_operation_block_t transactionBlock = [[self class] asyncDisplayBlockWithBounds:bounds
                                                                         ...
                                                                         expectedDisplaySentinelValue:displaySentinelValue
                                                                                      drawingDelegate:(id<TheodoliteAsyncLayerDrawingDelegate>)[self class]
